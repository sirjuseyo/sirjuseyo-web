...
const LOAN_BENEFITS = {
  ...
  "이벤트 대출": "빠른 대출과 크레딧 미션 할인 & 원금 미션 파격 인하(1%)!",
};
...
10만 원	100,000	원리현 상품	이벤트 대출	Special Seat (특별석)	${MONTH_CONFIG.event.period}	${MONTH_CONFIG.event.name}	원리포 상품	4,500	0%	4,500	1%	1,000	1%	1,000	1%	1,000	1%	1,000	6,500
20만 원	200,000	이포 상품	이벤트 대출	Special Seat (특별석)	${MONTH_CONFIG.event.period}	${MONTH_CONFIG.event.name}	원리포 상품	4,500	0%	4,500	1%	2,000	1%	2,000	1%	2,000	1%	2,000	8,500
...
30만 원	300,000	원리포 상품	이벤트 대출	Special Seat (특별석)	${MONTH_CONFIG.event.period}	${MONTH_CONFIG.event.name}	원리포 상품	4,500	0%	4,500	1%	3,000	1%	3,000	1%	3,000	1%	3,000	10,500
...
function parsePointSummaryTable(raw) {
  ...
  return dataLines.map((line) => {
    ...
    const normalizedOption = normalizeTableOption(loanOption);
    const FLAT_CREDIT_OPTIONS = ['웨이팅 대출', '롸잇나우 대출', '블랙찬스 티켓'];
    const isFlat = FLAT_CREDIT_OPTIONS.includes(normalizedOption);
    const flatBase = isFlat ? 4500 : toNumber(baseCreditPoints);
    const discountPct = toNumber(creditDiscountRate);
    const flatCreditDiscount = isFlat
      ? Math.round(flatBase * (1 - discountPct / 100))
      : toNumber(creditDiscountPoints);

    return {
      amountText,
      amountValue: toNumber(amountValue),
      baseRepayment,
      loanOption: normalizedOption,
      seat,
      loanHistory,
      loanStatus,
      changedRepayment,
      baseCreditPoints: flatBase,
      creditDiscountRate,
      creditDiscountPoints: flatCreditDiscount,
      baseInterestRate,
      ...
      totalPoints: isFlat ? 0 : toNumber(totalPoints),
    };
  });
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateResults() {
    ...
    if (principalRateLine) {
      if (summaryRow) {
        const isEventLoan = summaryRow.loanOption === "이벤트 대출";
        principalRateLine.textContent = isEventLoan
          ? `파격 인하, ${summaryRow.principalDiscountRate}`
          : formatChangedRate(summaryRow.principalDiscountRate, summaryRow.principalDiscountPoints);
      } else {
        principalRateLine.textContent = "-";
      }
    }
    ...
    if (summaryCredit) {
      if (summaryRow) {
        const isEventLoan = summaryRow.loanOption === "이벤트 대출";
        if (isEventLoan) {
          const pct = Math.round(summaryRow.creditDiscountPoints / summaryRow.amountValue * 1000) / 10;
