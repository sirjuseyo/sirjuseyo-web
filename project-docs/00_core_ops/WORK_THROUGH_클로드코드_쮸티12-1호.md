...
sirjuseyoWeb T-005~T-130 웹 통합 — 8월 썸머 베케이션 Ⅱ 대출 전체 통합 완료했습니다.
`feature/T-028-popup-html-content-update` 원격 푸시 완료했고 PR은 `#14`입니다.

작업 내용:
- T-005~T-010: 6월 대출 페이지 초기 작업 (폰트 18px 통일, legal-shared.js 연결, 법적고지 간격)
- T-011~T-019: 대출 가능성 검사기 초기 세팅 (앱 사이즈 480px, 1열 전환, 이미지 숨김, 폰트 통일)
- T-020~T-023: legal-shared.js 경로 수정, footer.js 교체, 연체금리 문구 수정
- T-024~T-026: 홈 강제 고지 팝업 추가 (monthly-loan + sirjuseyoWeb), 팝업 문구 2건 수정
- T-027~T-028: 팝업 활용% 전면 개편 + 예시 수치 변경 (HTML 4개 반영)
- T-029~T-031: 쇼핑 대출 페이지 — 누락 콘텐츠 추가, SEC-10 삭제, CTA 버튼명 변경
- T-032: .nojekyll 추가 (GitHub Pages Jekyll 비활성화)
- T-033~T-034: 홈페이지 원본 백업 + 지자체 실사 대비 위장 전환
- T-035~T-046: 홈페이지 4차 수정 (그룹핑·컬러코딩·배너) + 홈 이미지 누락 보완
- T-072: 햄버거 메뉴 menu.js 신규 생성 + 1Depth 드로어 UI 구현
- T-073: 팝업 하루 1회 재노출 방지 (localStorage)
- T-074~T-075: menu.js B안 카드 타일 디자인 + 햄버거 버튼 CSS span 방식
- T-076~T-079: nanocredit·loan-match 앱 사이즈 전환 + index-dev.html 신규 생성
- T-080: menu-dev.js 신규 생성 + 전체 DEV 파일 적용
- T-081~T-082: popup.js 신규 생성 (IIFE 분리·ID 충돌 수정) + 전체 페이지 적용
- T-083: 꿀정보(tip) 대표 페이지 신규 생성
- T-084: legal-shared.js 상단 고지 풀사이즈 통일
- T-085: 챌린지 대표 페이지 신규 생성
- T-086: 전체 페이지 ← 뒤로가기 네비 통일 (menu.js 통합)
- T-087: PRD body class="page" 추가 (legal-shared.js 고지 미표시 수정)
- T-088: 상단 고지↔콘텐츠 간격 통일
- T-089: JS 연결 순서 표준화 (popup.js → menu-dev.js → legal-shared.js)
- T-090: 대출 가능성 검사기 PRD 앱 사이즈 480px 적용
- T-091: 대출 가능성 검사기 PRD→DEV 정합 (이미지 제거 + 1열)
- T-092: 대출 검사기 DEV 배너 앱 사이즈(480px) + 스크롤 고정
- T-093: nanocredit·loan-match DEV 배너 앱 사이즈(480px) 고정
- T-094: 디렉토리 개편 1단계 — 월별대출 2026-04~07 + apply → monthly-loan/ 통합
- T-095: menu.js·menu-dev.js history.back() 버그 수정 (window 한정)
- T-096: 디렉토리 개편 3단계 — apply-review·loan-checker·unsuspend → tip/ 통합
- T-097~T-098: 메뉴 드로어 헤더 + 본체 480px 고정 (PC 풀사이즈 수정)
- T-099: 나노크레딧 상품 상세 페이지 전면 개편 (10/20/30/50 — 4개)
- T-100: 나노크레딧 "이달의 대출 신청하러 가기" 링크 수정
- T-101: 나노크레딧 수치 전면 업데이트 (7월 정책 반영) + DEV 4개 신규 생성
- T-102: 꿀정보 연체이력해제 페이지 통합 개편 (PRD + DEV 신규)
- T-104: 2026-07 대출 페이지 DEV 배너 풀사이즈 수정
- T-105: Deep Dive apply-review-dev.html UI 통합
- T-106: Deep Dive 진입 보안코드 overlay 구현 (항상 표시)
- T-107: 소통거리 제출 가이드 nanocredit→tip 이동 + 꿀정보 06번 + 나노크레딧 CTA
- T-108: 소통거리 등·초본→초본 텍스트 수정
- T-109: 소통거리 DEV 버전 15개 신규 생성 + index-dev.html 링크 연결
- T-110: 소통거리 SEC-12·13 섹션 타이틀 서브라인 추가
- T-113: 홈 화면 히어로 영역 문구 변경 (신용점수→미션·대안신용, 5회차)
- T-114: 전체 페이지 브레드크럼 추가 (menu.js·menu-dev.js + 71개 HTML)
- T-115: 한도 상향 Coming Soon 페이지 신규 생성
- T-116: 공지사항 안내 페이지 신규 생성
- T-118: 대출 검사기 ⑤신청 정보 URL month-config.js 자동 주입 + CLAUDE.md 체크리스트
- T-119: 원리포 상품의 메리트 페이지 신규 생성 (PRD + DEV)
- T-120: 원리포 계산기 신규 생성 + index 링크 변경
- T-121: 원리포 계산기 UI 디테일 수정 3종
- T-123: 8월 대출 페이지 index-dev.html 신규 생성 + 8월 전환 (3단계 체크리스트)
- T-124: 8월 대출 페이지 회차 수정 9건
- T-125: 홈 화면 7월→8월 전환 (live-badge·gift-box·타이틀·href)
- T-126: 대출 검사기 30만 원 이벤트 대출 비활성 + 문구 수정
- T-127: 대출 검사기 대표 거절 사유 4번 추가 (⑦안내섹션 + 결과패널)
- T-128: 8월 대출 페이지 대표 거절 사유 4번 문구 수정
- T-129: 대출 검사기 상단 안내 문구 워딩 수정
- T-130: 신청 폼 7월→8월 워딩 변경 + 이벤트 대출 30만 경고 추가 + 경고 문구 수정

검증:
- 사장님 VS Code Live Server 전체 테스트 완료
- DEV(-dev.html) / PRD(.html) 양쪽 적용 완료
- 디렉토리 개편 후 참조 경로 전수 수정 완료

커밋:
- `6b0e53a` T-005: 6월 대출 페이지 초기 작업
- `af8d679` T-011: 월드컵 챌린지 법적고지
- `ab9eac8` T-020: legal-shared.js 경로 수정
- `a1660d1` T-022: 연체금리 문구 수정
- `47e7965` T-024: 홈 강제 고지 팝업
- `5043427` T-025: sirjuseyoWeb 팝업
- `3e0cb64` T-028: 팝업 수치 반영
- `7ad5593` T-029: 쇼핑 대출 콘텐츠
- `77d6d72` T-030: SEC-10 삭제
- `2370d21` T-031: CTA 버튼명
- `55bcf01` T-032: .nojekyll
- `34ad7bb` T-033: 홈 백업
- `bc31103` T-034: 홈 위장
- `cbca6c2` T-035~T-044: 홈 4차 수정
- `20126dd` T-045/T-046: 홈 이미지
- `0d26a54` T-072: 햄버거 메뉴 신규
- `9bf803d` T-073: 팝업 재노출 방지
- `e833bdc` T-074: B안 카드 타일
- `5813f2c` T-075: 햄버거 CSS span
- `c3638fe` T-076/T-077: 앱 사이즈 전환
- `954c9ad` T-078/T-079: index-dev 신규
- `6015e2b` T-080: menu-dev.js 신규
- `61bb08b` T-081/T-082: popup.js 분리
- `38da632` T-083: 꿀정보 대표 페이지
- `fad4793` T-084: 고지 풀사이즈
- `d049be8` T-085: 챌린지 대표 페이지
- `e671798` T-086: 뒤로가기 통일
- `d4ec4a9` T-087: PRD class 추가
- `c401d33` T-088: 고지 간격
- `e2fb46b` T-089: JS 순서 표준화
- `412c3c2` T-090: PRD 480px
- `aee1e72` T-091: PRD→DEV 정합
- `b549c35` T-092: DEV 배너 480px
- `4fef5cf` T-093: nanocredit DEV 배너
- `bc93486` T-094: 디렉토리 1단계
- `af48274` T-095: history.back() 수정
- `69e20e2` T-096: 디렉토리 3단계
- `2ffda52` T-097: 드로어 헤더 480px
- `95b6cf0` T-098: 드로어 본체 480px
- `af2879c` T-099: 나노크레딧 개편
- `29a7ec0` T-100: 나노크레딧 링크
- `8919686` T-101: 나노크레딧 수치
- `d81803a` T-102: 연체이력해제
- `c6e47dc` T-104: DEV 배너 수정
- `684fc43` T-105: Deep Dive UI
- `461097e` T-106: 보안코드 overlay
- `9f450d9` T-107: 소통거리 tip 이동
- `b37d348` T-108: 초본 수정
- `d21c053` T-109: 소통거리 DEV 신규
- `2880556` T-110: 서브라인 추가
- `7fdde17` T-113: 홈 히어로 문구
- `79e511c` T-114: 브레드크럼
- `43b70e2` T-115/T-116: 한도상향·공지사항
- `5846239` T-118: URL 자동 주입
- `4cc5c56` T-119: 원리포 메리트
- `34c81ad` T-120: 원리포 계산기
- `3f06a2d` T-121: 원리포 UI 수정
- `b309222` T-123: 8월 대출 신규
- `519351c` T-124: 8월 회차 수정
- `487be1e` T-125: 홈 8월 전환
- `a5562d8` T-126: 30만 비활성
- `ded299d` T-127: 거절사유 4번
- `6f857fd` T-128: 거절사유 문구
- `189ae41` T-129: 안내 문구
- `a9b2747` T-130: 신청폼 8월
...
한 줄 버전:
- `sirjuseyoWeb T-005~T-130 웹 통합 완료, feature/T-028-popup-html-content-update 푸시 및 PR #14 생성 완료, 사장님 VS Code Live Server 전체 테스트 완료했습니다. 깃 & 배포 관리자님 feature -> dev 검토 부탁드립니다.`
```
...
1. 워딩 3곳: `7️⃣🈷️ 썸머 🏖️베케이션 대출` → `8️⃣🈷️ 썸머 🏝️베케이션 Ⅱ 대출`
   - h1 `.landing-title`, 현재 고객님 안내 문구, API 전송 문자열 4번 항목
2. `selectMethodEvent()`: `state.amount === '50만 원'` → `state.amount === '50만 원' || state.amount === '30만 원'`
...
### 변경 내용
1. `.tone-note-title`: `대출을 받을 수 있는 [써주세요. 앱 > 인기상품은 나노크레딧]은 무작정 신청하지 마세요.`
   → `[써주세요. 앱 > 인기상품은 나노크레딧]을 무작정 신청하지 마세요.`
2. `.tone-note-body` 1행: `이 검사기는 써주세요.의 실제 대출 운영 절차를 그대로 옮긴 시뮬레이터입니다.`
...
- **상태:** 테스트 완료(DONE) — 2026-08-01
- **로컬 커밋:** `6f857fd` (코드 커밋)
- **대상 파일:** `monthly-loan/2026-08/index-dev.html`

### 변경 내용
- 4번 항목 b 문구: `1년 이내 3개월 이상 무직 또는 과거 6개월 이상 무직` → `최근 3개월 이상 무직 또는 과거 6개월 이상 무직`
- em 추가: `노동 상태와 의지를 중요시 여깁니다.`
- 참조: 대출 가능성 검사기(T-127) 동일 문구로 통일

### 테스트
- VS Code Live Server → `monthly-loan/2026-08/index-dev.html` 테스트 완료
...
### 변경 내용
1. ⑦ 대표 거절 사유 안내 섹션 (`reject__list`) — 4번 추가
2. 결과 패널 (`reject__list`) — 4번 추가 (두 곳 동일)
   - 추가 문구: **최근 3개월 이상 무직 또는 과거 6개월 이상 무직**
...
## WT-137 · T-126 / W-126 [대출 검사기] 30만 원 이벤트 대출 비활성 + 문구 수정
...
### 변경 내용
1. `updateAmountWarnings()` JS 함수: `is30` 조건 추가
   - 30만 원 선택 시 이벤트 대출 옵션 `disabled = true` + 선택 리셋
   - 기존 50만 원 로직과 동일 방식 (`is50 || is30`)
2. `eventAmountNote` 문구: `10만 원, 20만 원만 선택 가능합니다. (30만 원, 50만 원 제외)`

---

## WT-136 · T-125 / W-125 [홈 화면] 8월 전환 (7월→8월)

### 작업 요약
- **상태:** 테스트 완료(DONE) — 2026-07-31
- **로컬 커밋:** `487be1e`
...
| 항목 | 변경 전 (7월) | 변경 후 (8월) |
|------|-------------|-------------|
| live-badge | `7월 대출` | `8월 대출` |
| gift-box 이미지 | `2026-07/assets/Palm-Surf_Jul-Loan.png` | `2026-08/assets/Plane-Luggage_Aug-Loan.png` |
| 메인 타이틀 | `7️⃣🈷️ 썸머 🏖️베케이션` | `8️⃣🈷️ 썸머 🏝️베케이션 Ⅱ` |
| 신청하기 href | `./monthly-loan/2026-07/index-dev.html` / `2026-07/` | `./monthly-loan/2026-08/index-dev.html` / `2026-08/` |

- popup.js 변경 없음 (사장님 지시: 홈 화면만 수정)

---

## WT-135 · T-124 / W-124 [8월 대출] 대출 페이지 회차 수정 (9건)
...
| # | 항목 | 변경 내용 |
|---|------|---------|
| 1 | 이벤트박스 10만 원 하단 | specialty-benefits 스타일로 안내 문구 2개 추가 |
| 2 | 이벤트 대출 h4 | `빠르게 대출을 받고 싶을때!` → `대출을 빠르게 받고 싶다면 선택!!` |
| 3 | 웨이팅 대출 h4 | `포인트 적립이 어렵고 힘들다면 선택!` → `포인트 적립이 힘들다면 선택!` |
| 4 | 롸잇나우 대출 h4 | `한도상향도 필요하고 빠르게 대출도 받고 블랙찬스 티켓도 받고 싶을 때!` → `한도상향이 필요하면 선택!` |
| 5 | 블찬 문구 2곳 | 1줄 → 블찬원/블찬이/블찬현 3줄 방식으로 교체 (2곳 동시) |
| 6 | 크레딧미션 악성 연체 | `악성 연체자 여부` → `악성 연체 여부 -> 악성 연체 신청자는...` |
| 7 | 크레딧미션 연체 원인 | 2개 `<li>` → 1개로 합치기 (`-->` 연결) |
| 8 | 웨이팅 섹션 h2 | `대신에 이거 하나만 해주세요!` → `포인트 구독을 해주셔야 합니다!` |
| 9 | 대표 거절 사유 | 4번 추가: `1년 이내 3개월 이상 무직 또는 과거 6개월 이상 무직` |

### 다음 단계
1. 사장님 로컬 테스트 (VS Code Live Server)
2. 테스트 완료 → ①DONE 표기 ②커밋 해시 ③작업현황 요약표 ④원격 피처 브랜치 푸시

---

## WT-134 · T-123 / W-123 [8월 대출] 대출 페이지 index-dev.html 신규 생성 + 8월 전환
...
| 작업자 | 쮸티12-1호 |
| 상태 | 테스트 완료(DONE) — 2026-07-31 |
| 기준 파일 | `monthly-loan/2026-07/index-dev.html` |

### 작업 내용 (CLAUDE.md 3단계 체크리스트 동시 처리)

**① monthly-loan/2026-08/index-dev.html 신규 생성 (커밋 `b309222`)**

| # | 항목 | 변경 내용 |
|---|------|---------|
| 1 | h1 제목 | 7️⃣🈷️ 썸머 🏖️베케이션 대출 → 8️⃣🈷️ 썸머 🏝️베케이션 Ⅱ 대출 |
| 2 | 인트로 슬로건 | "잠시 멈춰도…" → "어디든 체크인, 언제든 출발" (A안) |
| 3 | 이벤트명 | 워터밤💦대출 → 호캉스🏨✈️대출 |
| 4 | 이벤트 상품 | 원리포 상품 → 호캉스 이포 상품 |
| 5 | 이벤트 금액 | 30만·20만·10만 → 10만·20만 2종 (30만 제거) |
| 6 | 이벤트 상품 설명 | 원금 중 일부 → 원금 1% |
| 7 | 비교 테이블 30만·50만 이벤트 행 | → "해당 없음" |
| 8 | 신청 기간 | 7/1~7/25 → 8/1~8/25 |
| 9 | 웨이팅 심사 기간 | 8/1~8/5 → 9/1~9/5 |
| 10 | 히어로 이미지 | Surfer-Sunset_Jul-Loan.jpg → Voyage-Sky_Aug-Loan-001.jpg |
| 11 | 절차 이미지 | waterbomb-jul-001.jpg → hotelvacance-aug-001.jpg |
| 12 | 예시 ❸ | 10·20·30·50만 → 10·20만 |
| 13 | 웨이팅 테이블 날짜 | 2026/07/XX → 2026/08/XX |

**② js/menu.js, js/menu-dev.js**: `CURRENT_MONTH = '2026-08'`
...
## WT-133 · T-122 / W-122 [8월 대출] 썸머 베케이션 Ⅱ 기획서 작성
...
| 작업일시 | 2026-07-30 |
| 작업자 | 쮸티12-1호 |
| 상태 | 기획서 v0.4 완성 / 이미지 에셋 수령 완료 / HTML 착수 대기 |
| 참조 파일 | `monthly-loan/2026-07/index.html`, `PLAN_2026-07_썸머베케이션대출_기획서.md` |

### 작업 내용

**A. 기획서 v0.1 작성 (커밋 `b3fdc76`)**
- 파일: `project-docs/120_plan/PLAN_2026-08_썸머베케이션Ⅱ_기획서.md` (신규 생성)
- 7월 기획서 구조 기반으로 8월 변동 사항 전면 반영

**B. 8월 확정 변동 사항 (사장님 컨펌 → v0.2, 커밋 `8428d7d`)**

| # | 항목 | 7월 | 8월 |
|---|------|-----|-----|
| 1 | 상품명 | 7️⃣🈷️ 썸머 🏖️베케이션 대출 | **8️⃣🈷️ 썸머 🏝️베케이션 Ⅱ 대출** |
| 2 | 신청 기간 | 07/01 ~ 07/25 | **08/01 ~ 08/25** |
| 3 | 웨이팅 심사 기간 | 08/01 ~ 08/05 | **09/01 ~ 09/05** |
| 4 | 이벤트명 | 워터밤💦대출 | **호캉스🏨✈️대출** (✈️대출 붙여쓰기) |
| 5 | 이벤트 상품 | 원리포 상품 | **호캉스 이포 상품** (원금 1%+이자 전부 포인트) |
| 6 | 이벤트 금액 | 10만·20만·30만 (3종) | **10만·20만 (2종) — 30만 제외** |
| 7 | 비교 테이블 30만·50만 | 이벤트 대출 값 있음 | **"해당 없음"** |

**C. 인트로 슬로건 확정 (v0.3, 커밋 `09ee8ed`)**
- A안 확정: "어디든 체크인, 언제든 출발" / `8월 썸머 베케이션 Ⅱ, [써주세요.]가 함께합니다!`

**D. 이미지 에셋 수령 및 assets 배치 (v0.4, 커밋 `c685ae8`)**

| # | 파일명 | 용도 | 사이즈 |
|---|--------|------|--------|
| 1 | `Voyage-Sky_Aug-Loan-001.jpg` | 월별 대출 페이지 상단 히어로 | 1686×933 RGB |
| 2 | `hotelvacance-aug-001.jpg` | 월별 대출 페이지 하단 이벤트 | 1686×933 RGB |
| 3 | `Plane-Luggage_Aug-Loan.png` | 홈 gift-box 우측 상단 | 1536×1024 RGBA |
| 4 | `banner_voyage-aug-001_gitlab.jpg` | 앱 배너 (쮸티 범위 외) | 1983×793 RGB |

- `monthly-loan/2026-08/assets/` 폴더 신규 생성, 4종 배치 완료
- 기획서 섹션 6 이미지 에셋 표 신규 작성, 7-1 이미지 src 확정, 미결 사항 완료 처리

### 다음 단계
1. `monthly-loan/2026-08/index-dev.html` 신규 생성 (7월 파일 기반)
2. 테스트 완료 후 → `index.html` PRD 생성
3. CURRENT_MONTH 변경 + month-config.js 2026-08 추가
