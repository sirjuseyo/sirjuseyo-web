...
| 작업일시 | 2026-07-30 |
| 작업자 | 쮸티12-1호 |
| 상태 | 기획서 초안 완성 / 사장님 컨펌 후 HTML 착수 예정 |
...
| 4 | 이벤트명 | 워터밤💦대출 | **호캉스🏨✈️ 대출** |
| 5 | 이벤트 금액 | 10만·20만·30만 (3종) | **10만·20만 (2종) — 30만 제외** |
| 6 | 이미지 | 에셋 제공됨 | **사장님 ChatGPT 제작 후 전달 예정** |

**C. 미결 사항**
- 30만·50만 비교 테이블에서 이벤트 대출 행 표기 방식 → 사장님 컨펌 필요
- 인트로 슬로건 문구 → 사장님 확정 필요
- 이미지 파일명 → 사장님 전달 후 확정

### 다음 단계
1. 사장님 기획서 컨펌
2. `monthly-loan/2026-08/index-dev.html` 신규 생성 (7월 파일 기반)
3. 테스트 완료 후 → `index.html` PRD 생성
4. CURRENT_MONTH 변경 + month-config.js 2026-08 추가
