...
| W-100 | P1 | T-095 | **진행중(In-Progress)** — | **[잠복 버그 수정] 월별 대출 신청 페이지 상단 ← 네비 무반응** | `js/menu.js`, `js/menu-dev.js` | apply.html `const history = []` 전역 렉시컬 바인딩 → menu 스크립트 bare `history.back()` shadowing → `TypeError: history.back is not a function` → 클릭 핸들러 중단 / 두 파일 105행 → `window.history.back()` 한정 수정 (2파일 각 1행) | 2026-07-26 | 쮸티12-1호 |
