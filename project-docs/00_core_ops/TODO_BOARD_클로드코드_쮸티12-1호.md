...
| **T-073** | WT-086 | W-078 | `9bf803d` | **코딩완료(TEST WAIT)** | **[메인 DEV 팝업] 재노출 방지 활성화 (하루 1회)** | 쮸티12-1호 | 2026-07-17 | 【파일】`index-dev.html` / localStorage `sjy_popup_hidden` 체크·저장 로직 추가 — PRD와 동일 방식 |
...
| W-078 | P1 | T-073 | **코딩완료(TEST WAIT)** `9bf803d` | **[메인 DEV 팝업] 재노출 방지 활성화 (하루 1회)** | `index-dev.html` | DEV 팝업 재노출 방지 비활성화 상태 → PRD와 동일하게 하루 1회 제한으로 변경 — 테스트 반복 시 팝업이 매번 열리는 불편 해소 | 2026-07-17 | 쮸티12-1호 |
