...
| W-091 | P1 | - | **대기** | **[UI 통일] BACK 버튼 스타일 불일치 — 단일 스타일로 통일** | `loan-checker/index-dev.html`, `loan-match/index-dev.html`, `nanocredit/index-dev.html` | `← BACK` (loan-checker) vs `⬅ BACK` (loan-match·nanocredit) 혼재 → "이전" 또는 `← BACK` 단일 스타일로 통일 | 2026-07-22 | 쮸티12-1호 |
